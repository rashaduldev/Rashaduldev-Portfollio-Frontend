    ...
    <AnimatePresence>
      {isReady && !isOnline && (
        <motion.div
          ...
          className="fixed inset-0 z-[200] grid place-items-center overflow-hidden bg-background px-4 py-8 text-foreground"
          role="alertdialog"
          aria-modal="true"
        ...
        >
          <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
            ...
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_center,color-mix(in_oklab,var(--primary)_10%,transparent),transparent_60%)]" />
          </div>
          ...
          <motion.section
            ...
            className="relative w-full max-w-lg overflow-hidden rounded-[2rem] border border-primary/20 bg-card/90 p-6 text-center shadow-2xl shadow-primary/10 backdrop-blur-2xl sm:p-9"
          >
            <div className="mx-auto mb-6 flex w-fit items-center gap-2 rounded-full border border-primary/25 bg-primary/10 px-3 py-1.5 text-xs font-semibold uppercase tracking-[0.18em] text-primary">
            ...
            <div className="relative mx-auto mb-7 grid h-32 w-32 place-items-center" aria-hidden="true">
              ...
              />
              <div className="grid h-20 w-20 place-items-center rounded-3xl bg-gradient-to-br from-primary to-orange-500 text-primary-foreground shadow-lg shadow-primary/25">
            ...
            <p id="offline-description" className="mx-auto mt-4 max-w-md text-sm leading-6 text-muted-foreground sm:text-base">
            ...
            <button
              ...
              disabled={isChecking}
              className="mt-7 inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-2xl bg-primary px-5 py-3 font-bold text-primary-foreground shadow-lg shadow-primary/20 transition hover:brightness-110 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-2 focus-visible:ring-offset-background disabled:cursor-wait disabled:opacity-70"
            >
              <RefreshCw className={`h-5 w-5 ${isChecking ? "animate-spin" : ""}`} />
              {isChecking ? "Checking connection…" : "Try again"}
            </button>

            <p className={`mt-3 min-h-5 text-sm ${retryFailed ? "text-amber-600" : "text-muted-foreground"}`} aria-live="polite">
